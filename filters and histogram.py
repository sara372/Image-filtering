...
class MainWindow(qtw.QMainWindow):
    ...
    def display_pic(self, ui_element, pic, pic_name):
        ...
        # plt.show()
        pixmap = QPixmap('{}.jpg'.format(pic_name)).scaled(600,390)
    ...
    def low_filter_frequency(self):
        ...
        # remove salt & papper _out layers
    def median_filter(self):
        ...
        #first derivative _edge detection
    def laplacian_filter(self):
        """Applies laplacian filter on pics"""

        img_edges = np.mean(imread(self.fileName), axis=2)
        
        self.filtered_img = cv2.Laplacian(img_edges, cv2.CV_64F, ksize=3)# depth 
        self.filtered_img = cv2.convertScaleAbs(self.filtered_img) # absolute
